...
describe("worker boundaries", () => {
  it("uses stable, scoped job ids for idempotency", () => {
    const input = { workspaceId: "08e54170-f91d-4f40-a46f-c70a20f5a0b9", idempotencyKey: "request-00000001" };
  ...
  it("fails explicitly when AI has not been configured", async () => {
    const provider = createAiProvider(readWorkerConfig({ NODE_ENV: "test", AI_PROVIDER: "disabled" }));
    await expect(provider.complete({ system: "test", prompt: "test", maxOutputTokens: 10 })).rejects.toBeInstanceOf(
