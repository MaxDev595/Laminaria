...
export async function registerAuthRoutes(
  ...
  config: Pick<
    AppConfig,
    "sessionCookieName" | "sessionTtlSeconds" | "nodeEnv" | "google" | "webAppUrl"
  >,
): Promise<void> {
  app.get(
    "/v1/auth/providers",
    ...
    async (request, reply) => {
      ...
      if (!config.google) {
        return reply.redirect(
          `${config.webAppUrl}/${query.locale}/sign-in?error=google_not_configured`,
        );
      }
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      if (
        ...
        !query.code ||
        !query.state ||
        !sameToken(request.cookies[GOOGLE_STATE_COOKIE], query.state)
      ) {
  ...
  app.post(
    "/v1/auth/resend-verification",
    {
      ...
      schema: {
        tags: ["Authentication"],
        summary: "Resend email verification without account enumeration",
      },
    },
    async (request, reply) => {
  ...
  app.post(
    "/v1/auth/forgot-password",
    {
      ...
      schema: {
        tags: ["Authentication"],
        summary: "Request password reset without account enumeration",
      },
    },
    async (request, reply) => {
  ...
  app.post(
    "/v1/auth/reset-password",
    {
      ...
      schema: {
        tags: ["Authentication"],
        summary: "Consume reset token and revoke existing sessions",
      },
    },
    async (request, reply) => {
