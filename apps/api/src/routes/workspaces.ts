...
const createWorkspaceSchema = z.object({
  ...
  slug: z.string().trim().toLowerCase().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/).min(3).max(63),
});

export async function registerWorkspaceRoutes(app: FastifyInstance, repositories: UnitOfWork): Promise<void> {
    ...
    schema: {
      ...
      body: {
        ...
        properties: {
          ...
          slug: { type: "string", minLength: 3, maxLength: 63, pattern: "^[a-z0-9]+(?:-[a-z0-9]+)*$" },
        },
      },
    },
  }, async (request, reply) => {
  ...
  app.get<{ Params: { workspaceId: string } }>("/v1/workspaces/:workspaceId", {
