...
const registerSchema = z.object({
  ...
  name: z
    ...
    .min(1)
    .max(100)
    .transform((value) => normalizePlainText(value, 100)),
...
export async function registerPublicRoutes(
  ...
  app.post<{ Params: { slug: string } }>(
    "/v1/public/webinars/:slug/registrations",
    {
  ...
  app.post<{ Params: { slug: string } }>(
    "/v1/public/webinars/:slug/prejoin",
    {
      config: { rateLimit: { max: 30, timeWindow: "5 minutes" } },
      schema: {
        tags: ["Public webinars"],
        summary: "Issue short-lived LiveKit and realtime credentials",
      },
    },
    async (request) => {
      const { slug } = slugParams.parse(request.params);
...
function publicProjection(
  webinar: Awaited<ReturnType<PublicRegistrationService["publicWebinar"]>>,
) {
  return {
    slug: webinar.slug,
    title: webinar.title,
