...
export function VerifyEmail({ token, sent }: { token?: string; sent?: boolean }) {
  ...
  const [state, setState] = useState<"waiting" | "verifying" | "success" | "error">(
  ...
  const body =
    state === "verifying"
      ...
      : state === "success"
        ? locale === "ru"
          ? "Теперь создадим рабочее пространство."
    ...
    >
      ...
      {state === "success" ? (
        <Link href="/onboarding">
          <Button>{locale === "ru" ? "Создать пространство" : "Create workspace"}</Button>
        </Link>
      ) : state === "error" ? (
        <Link href="/sign-in">
