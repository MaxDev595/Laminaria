  ...
  const tabs = useMemo(
    () => [
      { id: "profile" as const, icon: UserRound, label: ru ? "Профиль" : "Profile" },
      {
        ...
        icon: UsersRound,
        label: ru ? "Рабочее пространство" : "Workspace",
      },
  ...
  return (
    <div className="dashboard-page settings-center">
      <PageHeading
        ...
        body={
          ru
            ? "Аккаунт, пространство и параметры ваших эфиров — в одном месте."
            : "Your account, workspace and webinar defaults in one place."
        }
      />
      ...
      {notice ? (
        <div className="settings-toast" role="status">
          <Check size={17} />
          <span>{notice}</span>
          <button type="button" onClick={() => setNotice("")} aria-label={ru ? "Закрыть" : "Close"}>
            <X size={16} />
          </button>
        </div>
      ) : null}
        ...
        <main className="settings-panel">
          ...
          {tab === "profile" ? (
            <ProfileSettings
              ...
              onSave={() =>
                saveProfile.mutate({ ...profile, avatarUrl: profile.avatarUrl.trim() || null })
              }
              onPassword={async () => {
                if (passwords.next !== passwords.confirm)
                  return setNotice(ru ? "Новые пароли не совпадают" : "New passwords do not match");
                try {
            ...
            <WorkspaceSettingsView
              ...
              onDelete={async () => {
                if (
                  !window.confirm(
                    ru
                      ? `Удалить «${workspace.name}» вместе со всеми вебинарами?`
                      : `Delete “${workspace.name}” and all its webinars?`,
                  )
                )
                  return;
                try {
          ...
          {tab === "webinars" ? (
            <WebinarSettings
              ...
              value={webinar}
              setValue={setWebinar}
              saving={saveWebinar.isPending}
              onSave={() => saveWebinar.mutate()}
            />
          ) : null}
          {tab === "polls" ? (
            <PollSettings
              ...
              value={polls}
              setValue={setPolls}
              saving={savePolls.isPending}
              onSave={() => savePolls.mutate()}
            />
          ) : null}
          {tab === "branding" ? (
            <BrandingSettingsView
              ...
              setWorkspace={setWorkspaceForm}
              value={branding}
              setValue={setBranding}
              saving={saveBranding.isPending}
              onSave={() => saveBranding.mutate()}
              setNotice={setNotice}
            />
          ) : null}
          ...
          {tab === "notifications" ? (
            <NotificationSettings
              ...
              onSave={() => void savePreferences({ notifications })}
            />
          ) : null}
          {tab === "devices" ? (
            <DeviceSettings
              ...
              value={devices}
              setValue={setDevices}
              onSave={() => void savePreferences({ devices })}
              setNotice={setNotice}
            />
          ) : null}
          {tab === "billing" ? (
            <BillingSettings
              ...
              billingStatusLoading={servicesQuery.isLoading}
            />
          ) : null}
          {tab === "security" ? (
            <SecuritySettings
  ...
  return (
    <section className="settings-stack">
      <SettingsHeader
        ...
        body={ru ? "Как вас видят участники и команда." : "How your team and attendees see you."}
      />
      <div className="settings-card settings-profile-card">
        <ImageUpload
          kind="avatar"
          value={value.avatarUrl}
          label={ru ? "Загрузить аватар" : "Upload avatar"}
          ru={ru}
          setNotice={setNotice}
          onChange={(avatarUrl: string) => setValue({ ...value, avatarUrl })}
        />
        <div className="settings-form-grid">
          <Field label={ru ? "Имя и фамилия" : "Full name"}>
            <input
              value={value.name}
          ...
          <Field
            ...
            hint={
              ru
                ? "Email аккаунта нельзя изменить здесь."
                : "Your account email cannot be changed here."
            }
          >
            <input value={email} disabled />
          </Field>
          <Field label={ru ? "Язык интерфейса" : "Interface language"}>
            <StyledSelect
              value={value.locale}
              ariaLabel={ru ? "Язык интерфейса" : "Interface language"}
              options={[
                { value: "ru", label: "Русский" },
                { value: "en", label: "English" },
              ]}
              onChange={(next) => setValue({ ...value, locale: next })}
            />
          </Field>
          <Field label={ru ? "Часовой пояс" : "Time zone"}>
            <StyledSelect
              value={value.timezone}
              ariaLabel={ru ? "Часовой пояс" : "Time zone"}
              options={zones.map((zone: string) => ({ value: zone, label: zone }))}
              onChange={(next) => setValue({ ...value, timezone: next })}
            />
          </Field>
        </div>
      ...
      >
        <h3>
          <KeyRound size={18} />
          {ru ? "Смена пароля" : "Change password"}
        </h3>
        <div className="settings-form-grid settings-form-grid--three">
          <input
            ...
            aria-hidden="true"
          />
          <Field label={ru ? "Текущий пароль" : "Current password"}>
            <input
              type="password"
          ...
          <Field label={ru ? "Новый пароль" : "New password"}>
            <input
              type="password"
              name="new-password"
          ...
          <Field label={ru ? "Повторите пароль" : "Confirm password"}>
            <input
              type="password"
        ...
        <div className="settings-actions">
          <Button type="submit" variant="secondary">
            {ru ? "Изменить пароль" : "Change password"}
          </Button>
        </div>
      </form>
    </section>
...
function WorkspaceSettingsView({
  ...
  return (
    <section className="settings-stack">
      <SettingsHeader
        icon={<UsersRound />}
        title={ru ? "Рабочее пространство" : "Workspace"}
        body={ru ? "Бренд и доступ вашей команды." : "Your team identity and access."}
      />
      <div className="settings-card">
        <div className="settings-form-grid">
          <Field label={ru ? "Название Workspace" : "Workspace name"}>
            <input
              value={value.name}
              disabled={!canManage}
              onChange={(e) => setValue({ ...value, name: e.target.value })}
          ...
          <Field label={ru ? "Логотип" : "Logo"}>
            <ImageUpload
              ...
              disabled={!canManage}
              setNotice={setNotice}
              onChange={(logoUrl: string) => setValue({ ...value, logoUrl })}
            />
          </Field>
        </div>
        <div className="settings-actions">
          <Button variant="secondary" onClick={onTeam}>
            <UsersRound size={17} />
            {ru ? "Участники и роли" : "Members and roles"}
          </Button>
          {canManage ? <SaveButton ru={ru} loading={saving} onClick={onSave} /> : null}
        </div>
      </div>
      {role === "OWNER" ? (
        <DangerCard
          title={ru ? "Удаление Workspace" : "Delete workspace"}
          body={
            ru
              ? "Вебинары и настройки пространства будут недоступны. Действие необратимо."
              : "Webinars and workspace settings will become unavailable. This cannot be undone."
          }
          action={ru ? "Удалить Workspace" : "Delete workspace"}
          onClick={onDelete}
        />
      ) : null}
    </section>
...
function WebinarSettings({ ru, value, setValue, saving, onSave }: any) {
  return (
    <section className="settings-stack">
      <SettingsHeader
        icon={<Video />}
        title={ru ? "Вебинары по умолчанию" : "Webinar defaults"}
        body={
          ru
            ? "Эти значения подставляются при создании нового вебинара."
            : "These values prefill every new webinar."
        }
      />
      <div className="settings-card">
        <div className="settings-form-grid">
          <Field label={ru ? "Язык вебинара" : "Webinar language"}>
            <StyledSelect
              value={value.language}
              ariaLabel={ru ? "Язык вебинара" : "Webinar language"}
              options={[
                { value: "ru", label: "Русский" },
                { value: "en", label: "English" },
              ]}
              onChange={(next) => setValue({ ...value, language: next })}
            />
          </Field>
          <Field label={ru ? "Часовой пояс" : "Time zone"}>
            <StyledSelect
          ...
          <Field label={ru ? "Доступ" : "Access"}>
            <StyledSelect
              value={value.access}
              ariaLabel={ru ? "Доступ" : "Access"}
              options={[
                { value: "PUBLIC", label: ru ? "Публичный" : "Public" },
                { value: "PRIVATE", label: ru ? "Приватный" : "Private" },
              ]}
              onChange={(next) => setValue({ ...value, access: next })}
            />
          </Field>
        </div>
        <div className="settings-toggle-list">
          <Toggle
            label={ru ? "Разрешать гостевой вход" : "Allow guest entry"}
            checked={value.allowGuests}
            onChange={(checked) => setValue({ ...value, allowGuests: checked })}
          />
          <Toggle
            label={ru ? "Требовать регистрацию" : "Require registration"}
            checked={value.requireRegistration}
            onChange={(checked) => setValue({ ...value, requireRegistration: checked })}
          />
          <Toggle
            label={ru ? "Автоматически запускать запись" : "Start recording automatically"}
            checked={value.autoRecording}
            onChange={(checked) => setValue({ ...value, autoRecording: checked })}
          />
          <Toggle
            label={ru ? "Разрешать чат зрителям" : "Allow attendee chat"}
            checked={value.viewerChat}
            onChange={(checked) => setValue({ ...value, viewerChat: checked })}
          />
        </div>
        <div className="settings-actions">
          <SaveButton ru={ru} loading={saving} onClick={onSave} />
          ...
          <Field label={ru ? "Показывать результаты" : "Show results"}>
            <StyledSelect
              ...
              options={[
                ...
                {
                  value: "AFTER_CLOSE",
                  label: ru ? "После закрытия опроса" : "After the poll closes",
                },
              ]}
    ...
    <section className="settings-stack">
      ...
      <div className="settings-card branding-settings-grid">
        ...
        <div
          ...
          style={{ "--preview-accent": value.accentColor } as React.CSSProperties}
        >
          {value.coverImageUrl ? (
      ...
      <ComingSoonCard
        ...
        body={
          ru
            ? "Собственный домен, скрытие Laminaria и фирменные email-письма появятся в Business."
            : "Custom domains, hidden Laminaria branding and branded email will arrive in Business."
        }
        items={
          ru
            ? ["Собственный домен", "Без Powered by Laminaria", "Оформление писем", "Фавикон"]
            : ["Custom domain", "No Powered by Laminaria", "Branded email", "Favicon"]
        }
      />
    </section>
  );
  ...
  return (
    <section className="settings-stack">
      <SettingsHeader
        ...
        body={
          ru
            ? "Интеграция Laminaria с сайтом, CRM или LMS."
            : "Connect Laminaria to your website, CRM or LMS."
        }
      />
      <ComingSoonCard
        ru={ru}
        ...
        body={
          ru
            ? "Запланирован для Business после запуска MVP. Сейчас раздел ничего не имитирует и не создаёт небезопасные ключи."
            : "Planned for Business after the MVP launch. This section does not create fake or insecure keys."
        }
        items={[
          "webinar.started",
          "webinar.ended",
...
function ComingSoonCard({
  ...
  return (
    <div className="settings-card business-coming-soon">
      <div className="business-coming-soon__icon">
        <LockKeyhole size={22} />
      </div>
      <div>
  ...
  const rows = [
    {
      ...
      label: ru ? "Подтверждение регистрации" : "Registration confirmation",
    },
    { key: "webinarReminder", label: ru ? "Напоминание перед вебинаром" : "Webinar reminder" },
    { key: "teamInvitation", label: ru ? "Приглашение в команду" : "Team invitation" },
    { key: "recordingReady", label: ru ? "Готовая запись" : "Recording ready" },
  ];
  return (
    <section className="settings-stack">
      <SettingsHeader
        ...
        body={
          ru
            ? "Выберите важные события, о которых стоит сообщать."
            : "Choose which important events should notify you."
        }
      />
      <div className="settings-card">
        <div className="settings-toggle-list">
          {rows.map((row) => (
            <Toggle
              ...
              onChange={(checked) => setValue({ ...value, [row.key]: checked })}
            />
          ))}
        </div>
        <div className="settings-actions">
          <SaveButton ru={ru} onClick={onSave} />
        </div>
      </div>
    </section>
  );
...
function DeviceSettings({ ru, value, setValue, onSave, setNotice }: any) {
  ...
  useEffect(() => {
    void navigator.mediaDevices
      ...
      .then((items) =>
        setAvailable({
          cameras: items.filter((x) => x.kind === "videoinput"),
          microphones: items.filter((x) => x.kind === "audioinput"),
          speakers: items.filter((x) => x.kind === "audiooutput"),
        }),
      )
      .catch(() => undefined);
  ...
  async function test() {
    try {
      ...
      const stream = await navigator.mediaDevices.getUserMedia({
        video: value.cameraId ? { deviceId: { exact: value.cameraId } } : true,
        audio: value.microphoneId ? { deviceId: { exact: value.microphoneId } } : true,
      });
      ...
      setNotice(
        ru
          ...
          : "Allow browser camera and microphone access",
      );
    }
  }
  function stop() {
  ...
  return (
    <section className="settings-stack">
      <SettingsHeader
        icon={<Camera />}
        title={ru ? "Камера и звук" : "Camera & audio"}
        body={
          ru
            ? "Устройства, которые будут выбраны перед эфиром."
            : "Devices selected when you enter a webinar."
        }
      />
      <div className="settings-card">
        <div className="settings-form-grid">
          <DeviceSelect
            icon={<Camera />}
            label={ru ? "Камера по умолчанию" : "Default camera"}
            value={value.cameraId ?? ""}
            items={available.cameras}
            fallback={ru ? "Системная камера" : "System default camera"}
            onChange={(cameraId: string) => setValue({ ...value, cameraId })}
          />
          <DeviceSelect
            icon={<Mic />}
            label={ru ? "Микрофон по умолчанию" : "Default microphone"}
            value={value.microphoneId ?? ""}
            items={available.microphones}
            fallback={ru ? "Системный микрофон" : "System default microphone"}
            onChange={(microphoneId: string) => setValue({ ...value, microphoneId })}
          />
          <DeviceSelect
            icon={<MonitorSpeaker />}
            label={ru ? "Динамики" : "Speakers"}
            value={value.speakerId ?? ""}
            items={available.speakers}
            fallback={ru ? "Системные динамики" : "System default speakers"}
            onChange={(speakerId: string) => setValue({ ...value, speakerId })}
          />
          <Field label={ru ? "Качество видео" : "Video quality"}>
            <StyledSelect
              value={value.videoQuality ?? "auto"}
              ariaLabel={ru ? "Качество видео" : "Video quality"}
              options={["auto", "360p", "480p", "720p", "1080p"].map((quality) => ({
                value: quality,
                label:
                  quality === "720p" ? "720p HD" : quality === "1080p" ? "1080p Full HD" : quality,
              }))}
              onChange={(videoQuality) => setValue({ ...value, videoQuality })}
            />
          </Field>
        </div>
        {testing ? (
          <div className="device-preview">
            ...
            <span>
              <span className="device-preview__live" />
              {ru ? "Устройства работают" : "Devices are working"}
            </span>
          </div>
        ) : null}
        <div className="settings-actions">
          <Button variant="secondary" onClick={testing ? stop : test}>
            <Camera size={17} />
            {testing
              ? ru
                ? "Завершить тест"
...
function DeviceSelect({ icon, label, value, items, fallback, onChange }: any) {
  return (
    <Field label={label}>
      <div className="settings-input-icon">
        ...
        <StyledSelect
          ...
          options={[
            ...
            ...items.map((item: MediaDeviceInfo, index: number) => ({
              value: item.deviceId,
...
function BillingSettings({
  ...
  const offers = [
    {
      ...
      features: ru
        ...
        : ["Up to 25 participants", "HD video", "Chat", "Screen sharing"],
    },
    {
      id: "professional",
      name: "Pro",
    ...
    },
    {
      ...
      features: ru
        ...
        : ["Up to 1,000 participants", "API", "White Label", "Team"],
    },
  ] as const;
  async function checkout(nextPlan: "professional" | "business") {
    ...
    setLoading(nextPlan);
    try {
      const result = await api.createBillingCheckout(workspaceId, {
        plan: nextPlan,
        interval,
        locale,
      });
  ...
  return (
    <section className="settings-stack">
      <SettingsHeader
        icon={<CreditCard />}
        title={ru ? "Тариф и оплата" : "Plan & billing"}
        body={
          ru
            ? "Безопасная оплата картой и управление подпиской."
            : "Secure card payments and subscription management."
        }
      />
      {!billingStatusLoading && !billingConfigured ? (
        <div className="billing-setup-notice">
      ...
      <div className="settings-plan-card">
        <div>
          <small>{ru ? "ТЕКУЩИЙ ТАРИФ" : "CURRENT PLAN"}</small>
          <strong>{plan === "PROFESSIONAL" ? "PRO" : plan}</strong>
          <p>
            {plan === "FREE"
              ? ru
        ...
        {plan !== "FREE" ? (
          <div className="settings-actions">
            <Button variant="secondary" onClick={() => void portal()} disabled={Boolean(loading)}>
              {loading === "portal" ? (
      ...
      <div
        ...
        role="group"
        aria-label={ru ? "Период оплаты" : "Billing interval"}
      >
        <button
          type="button"
        ...
        >
          {ru ? "Год" : "Year"}
          <span>{ru ? "выгоднее" : "best value"}</span>
        </button>
        <button
          type="button"
      ...
      <div className="billing-offers">
        {offers.map((offer) => {
          const current =
            plan === offer.id.toUpperCase() || (offer.id === "professional" && plan === "PRO");
          const paid = offer.id !== "free";
          const amount = interval === "year" ? offer.year : offer.month;
          return (
            <article
              key={offer.id}
              className={`${offer.id === "professional" ? "is-featured" : ""} ${current ? "is-current" : ""}`}
            >
              <header>
                <div>
                  <small>
                ...
                <div className="billing-price">
                  {offer.comingSoon ? (
                    <strong>{ru ? "Скоро" : "Soon"}</strong>
                  ) : (
                    <>
                      <strong>${amount}</strong>
                      <span>
                        {interval === "year" ? (ru ? "/год" : "/year") : ru ? "/месяц" : "/month"}
                      </span>
                    </>
                  )}
                </div>
              ...
              {paid ? (
                <Button
                  ...
                  disabled={offer.comingSoon || !billingConfigured || current || Boolean(loading)}
                >
                  {loading === offer.id ? (
                    <LoaderCircle className="spin" size={17} />
                  ) : (
                ...
                <Button variant="secondary" disabled>
                  {current ? (ru ? "Активен" : "Active") : ru ? "Бесплатно" : "Free"}
                </Button>
              )}
            </article>
          );
      ...
      <div className="settings-card">
        ...
        <div className="usage-grid">
          <Usage value={usage.members} label={ru ? "участников команды" : "team members"} />
          <Usage value={usage.webinars} label={ru ? "вебинаров" : "webinars"} />
          <Usage value={usage.recordings} label={ru ? "записей" : "recordings"} />
          <Usage value={formatBytes(usage.storageBytes)} label={ru ? "хранилище" : "storage"} />
        </div>
      </div>
    </section>
  );
...
function SecuritySettings({ ru, sessions, loading, onLogoutAll, onExport, onDelete }: any) {
  return (
    <section className="settings-stack">
      <SettingsHeader
        icon={<ShieldCheck />}
        ...
        body={
          ru ? "Сессии, экспорт и контроль аккаунта." : "Sessions, exports and account control."
        }
      />
      <div className="settings-card">
        <h3>{ru ? "Активные сессии" : "Active sessions"}</h3>
        {loading ? (
          <SettingsLoading />
        ) : (
            ...
            {sessions.map((session: any) => (
              <div key={session.id}>
                ...
                <div>
                  ...
                  <small>
                    {session.ipAddress ?? (ru ? "IP не определён" : "IP unavailable")} ·{" "}
                    {new Date(session.lastSeenAt).toLocaleString()}
                  </small>
                </div>
                {session.current ? (
                  <span className="session-current">{ru ? "Текущая" : "Current"}</span>
                ) : null}
              </div>
            ))}
          </div>
        ...
        <div className="settings-actions">
          <Button variant="secondary" onClick={onLogoutAll}>
            <LogOut size={17} />
            {ru ? "Выйти со всех устройств" : "Sign out everywhere"}
          </Button>
        </div>
      </div>
      <div className="settings-card">
        <h3>{ru ? "Ваши данные" : "Your data"}</h3>
        <p className="settings-card__body">
          {ru
            ? "Скачайте копию данных аккаунта в формате JSON."
            : "Download a JSON copy of your account data."}
        </p>
        <div className="settings-actions settings-actions--left">
          <Button variant="secondary" onClick={onExport}>
            <Download size={17} />
            {ru ? "Экспортировать данные" : "Export data"}
          </Button>
        </div>
      </div>
      <DangerCard
        title={ru ? "Удалить аккаунт" : "Delete account"}
        body={
          ru
            ? "Профиль будет удалён, а все активные сессии завершены. Действие необратимо."
            : "Your profile will be deleted and every session revoked. This cannot be undone."
        }
        action={ru ? "Удалить аккаунт" : "Delete account"}
        onClick={onDelete}
      />
    </section>
  );
...
function ImageUpload({ kind, value, label, onChange, ru, disabled = false, setNotice }: any) {
  ...
  async function selectFile(file: File | undefined) {
    ...
    if (
      ...
      setNotice(
        ru
          ? "Выберите PNG, JPEG или WebP размером до 2 МБ"
    ...
    try {
      ...
      onChange(result.url);
      setNotice(
        ru ? "Изображение загружено. Сохраните изменения." : "Image uploaded. Save your changes.",
      );
    } catch (error) {
      ...
      <div className="image-upload__actions">
        ...
        {value && !disabled ? (
          <button
            ...
            onClick={() => onChange("")}
            aria-label={ru ? "Удалить изображение" : "Remove image"}
          >
            <Trash2 size={16} />
          </button>
        ) : null}
