...
export function RegistrationConfirmation({ slug, token }: { slug: string; token?: string }) {
  ...
  const body =
    state === "loading"
      ? locale === "ru"
        ? "Проверяем защищённую ссылку…"
        : "Checking the secure link…"
      : state === "success"
        ? locale === "ru"
          ? "Пропуск сохранён. Открываем страницу ожидания."
          : "Your access pass is saved. Opening the waiting room."
        : error;
      ...
      <div className={`confirmation-card confirmation-card--${state}`}>
        ...
        {state === "success" ? (
          <Link href={`/w/${slug}/waiting`}>
            <Button>
              {locale === "ru" ? "Открыть ожидание" : "Open waiting room"}
              <ArrowRight size={17} />
            </Button>
          </Link>
        ) : state === "error" ? (
          <Link href={`/w/${slug}`}>
            <Button variant="secondary">
              {locale === "ru" ? "Вернуться к вебинару" : "Return to webinar"}
            </Button>
          </Link>
        ) : null}
      </div>
