...
export function WebinarWaitingRoom({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    setAccessToken(sessionStorage.getItem(`laminaria-access:${slug}`));
  }, [slug]);

  const webinar = query.data?.webinar;
  const targetTime = webinar?.scheduledStartAt ? new Date(webinar.scheduledStartAt).getTime() : null;
  const timeLeft = useMemo(() => calculateTimeLeft(targetTime, now), [targetTime, now]);
  const isLiveOrReady = webinar?.status === "LIVE" || (timeLeft?.totalMs ?? Number.POSITIVE_INFINITY) <= 0;
    ...
    return (
      <main className="dashboard-gate">
        ...
        <ServiceState
          ...
          action={<Button variant="secondary" onClick={() => void query.refetch()}>{locale === "ru" ? "Повторить" : "Try again"}</Button>}
        />
      </main>
    );
  }
  ...
  return (
    <main className="waiting-room">
      ...
      <motion.section className="waiting-card" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}>
        <div className="waiting-orb" aria-hidden="true"><span /><span /><span /></div>
        <Badge tone={isLiveOrReady ? "danger" : "primary"}>
          {isLiveOrReady ? (locale === "ru" ? "Можно входить" : "Ready to join") : (locale === "ru" ? "Ожидание вебинара" : "Webinar waiting room")}
        </Badge>
        <h1>{webinar.title}</h1>
        <p>{webinar.description || (locale === "ru" ? "Регистрация подтверждена. Эта страница обновляет время до старта." : "Registration confirmed. This page keeps the countdown fresh.")}</p>

        <div className="waiting-meta">
          <span><CalendarPlus size={17} />{webinar.scheduledStartAt ? formatLocalDate(webinar.scheduledStartAt, locale) : (locale === "ru" ? "Время уточняется" : "Time to be confirmed")}</span>
          <span><Clock3 size={17} />{localTimezone()}</span>
          <span><CheckCircle2 size={17} />{accessToken ? (locale === "ru" ? "Пропуск сохранён" : "Access pass saved") : (locale === "ru" ? "Нужна регистрация" : "Registration required")}</span>
        </div>
        ...
        {timeLeft && !isLiveOrReady ? (
          <>
            ...
            <div className="countdown-grid" aria-live="polite">
              ...
              <CountdownPart value={timeLeft.minutes} label={locale === "ru" ? "минут" : "minutes"} />
              <CountdownPart value={timeLeft.seconds} label={locale === "ru" ? "секунд" : "seconds"} />
            </div>
          </>
        ) : (
        ...
        <div className="waiting-actions">
          {accessToken ? (
            ...
            <Link href={`/w/${slug}`}>
              <Button size="lg">{locale === "ru" ? "Зарегистрироваться" : "Register"}<ArrowRight size={18} /></Button>
            </Link>
          )}
        </div>
      </motion.section>
